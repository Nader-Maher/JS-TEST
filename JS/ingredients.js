...
async function getIngr()
{
    ...
    let catApi =await fetch ("https://www.themealdb.com/api/json/v1/1/list.php?i=list")
    ...
    for ( let i=0 ; i<catMeals.length ; i++ )
    {
        ...
        CatHasalah += 
                    ` <div id="hoverr" class="col-md-6 hight col-lg-3  my-3 shadow  overflow-hidden ">
                            <div class=" text-center">
                                <i class="fa-solid fa-bowl-food fa-4x text-success"></i>
                                <h3 class="text-white">${catMeals[i].strIngredient}</h3>
                                <p class="text-white fs-6">${catMeals[i].strDescription}</p>
                            </div>
        ...
        document.querySelector(".ingrMeals").innerHTML = CatHasalah
